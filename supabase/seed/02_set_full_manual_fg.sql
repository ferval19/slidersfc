...
--
-- Es idempotente: se puede volver a aplicar y sólo actualiza.

begin;

-- Los valores, una sola vez. Tabla temporal en lugar de repetir la lista:
-- dos copias de lo mismo se desincronizan en cuanto alguien edita una.
create temporary table incoming_values (
  slug       text not null,
  applies_to text not null,
  value      int  not null
) on commit drop;

insert into incoming_values (slug, applies_to, value) values
  -- Velocidad
  ('sprint_speed', 'user', 33), ('sprint_speed', 'cpu', 32),
...
begin
  ...
  -- Si algún slug del documento no está en el catálogo, paramos: mejor fallar
  -- que publicar un set al que le faltan valores en silencio.
  select string_agg(format('%s (%s)', i.slug, i.applies_to), ', ')
    into missing
  from incoming_values i
  where not exists (
    select 1
    from public.slider_definitions d
    where d.game_id = target_game
      and d.slug = i.slug
      and d.applies_to = i.applies_to
  );

  if missing is not null then
    raise exception 'Estos sliders no están en el catálogo de fc26: %', missing;
  end if;
  ...
  -- duplicarlo al reaplicar el seed.
  ...
  select target_set, d.id, i.value
  from incoming_values i
  join public.slider_definitions d
  ...
  from public.slider_set_values
  where slider_set_id = target_set;
