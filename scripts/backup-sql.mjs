...
function setBlock({ set, owner, game, values, comments, versions, changes, definitionById, profileById }) {
  const lines = [
    ...
    '    (id, owner_id, game_id, title, slug, description, cpu_behaviour,',
    '     difficulty, half_length, camera, camera_height, camera_zoom,',
    '     version, is_published, created_at, updated_at)',
    '  values (',
    ...
    `    ${qn(set.difficulty)}, ${qn(set.half_length)}, ${qn(set.camera)},`,
    `    ${set.camera_height ?? 'null'}, ${set.camera_zoom ?? 'null'},`,
    ...
    '        half_length   = excluded.half_length,',
